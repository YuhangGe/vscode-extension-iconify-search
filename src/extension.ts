...
let store = new Map<string, IconSet>();
let storeKeys: string[] = [];
async function loadJsonStore() {
  if (store.size > 0) return true;
  const root = workspace.workspaceFolders?.[0].uri.fsPath;
  if (!root) {
    window.showErrorMessage("Please Open a File to Insert Iconify Icon");
    return false;
  }
  const iconifyJsonDir = path.join(
    root,
    "node_modules",
    "@iconify/json",
    "json"
  ...
  context.subscriptions.push(
    commands.registerCommand("iconify.search", async () => {
      ...
      // window.showInformationMessage(`Got: ${text}`);
			...
			window.showInformationMessage(`Got: ${matchedIcons.length}, ${hasMore}`);
